...
export class Service {
  ...
  private webSocket: WebSocket | null | WebSocketNode = null;
  ...
  public connectWebSocket(): void {
    if (typeof window === 'undefined') {
      this.webSocket = new WebSocketNode(this.waspWebSocketUrl);
      this.webSocket.on('message', (x) => this.handleIncomingMessage({ data: x.toString() } as MessageEvent<string>));
      this.webSocket.on('close', () => setTimeout(this.connectWebSocket.bind(this), 1000));
      this.webSocket.on('open', () => console.info(`Connecting to Websocket => ${this.waspWebSocketUrl}`));
    } else {
      ...
      this.webSocket.addEventListener('open', () =>
        console.info(`Connecting to Websocket => ${this.waspWebSocketUrl}`),
      );
    }
  }
